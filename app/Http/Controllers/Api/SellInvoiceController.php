...
use App\Models\SellInvoicePayment;
use App\Models\SellInvoice;
use App\Models\SellInvoiceDetail;
...
class SellInvoiceController extends Controller
{
    ...
    public function get_invoices_data(Request $request)
    {
        try {
            ...
            $order = SellInvoice::searchInvoice($_query)
                                ->searchFrom($_from)
                                ->searchTo($_to)
                                ->orderBy('id', 'DESC')
                                ->paginate(10);
    ...
    public function post_invoice_data(Request $request)
    {
        ...
        $cart = SellCart::where('user_id', $_user)->with('item.unit')->get();
        ...
        $so = [];
        $so['is_so'] = ($request->input('is_so') == 1) ? true : false;
        $so['so_number'] = $request->input('so_number');

        $order = new SellInvoice();
        ...
        if ($order->save()) {
            SellCart::where('user_id', $_user)->delete();
    ...
    public function get_cart_data(Request $request)
    {
        try {
            $_user = isset($request['user']) ? $request['user'] : '';
            return response()->json(SellCart::where('user_id', $_user)->with('item.unit')->orderBy('id', 'DESC')->get(), 200);
        } catch (Exception $e) {
            return response()->json(['error' => $e->getMessage()], 500);
        }
    }
    ...
    {
        $_user = isset($request['user']) ? $request['user'] : '';
        $check = SellCart::where('item_id', $request->input('item_id'))->where('user_id', $_user)->count();
        if ($check == 0) {
            $cart = new SellCart();
    ...
    public function get_payment_data(Request $request)
    {
        try {
            ...
            $payment  = SellInvoicePayment::where('sell_invoice_id', $_invoice)->get();
            $invoice  = SellInvoice::find($_invoice);
            $total = 0;
            if (count($payment) > 0) {
    ...
    {
        $pay = new SellInvoicePayment();
        $pay->sell_invoice_id = $request->input('sell_invoice_id');
        ...
        if ($pay->save()) {
            $invoice  = SellInvoice::find($pay->sell_invoice_id);
