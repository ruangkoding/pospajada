...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getCart() {
                this.totalHarga = 0;
                service.fetchData('/api/so/cart?user=' + this.userid)
                .then(response => {
            ...
            deleteData(id) {
                service.deleteData('/api/so/cart?id=' + id)
                .then(response => {
                    if (response.status === 'ok') {
                        $('#delete_modal').modal('hide');
                        this.$swal("Berhasil!", "Barang Berhasil Dihapus Dari Keranjang Belanja!", "success")
            ...
            checkOutCart() {
                ...
                if (validasi_checkout === true) {
                    ...
                    service.postData('/api/so/checkout?user=' + this.userid, this.checkout)
                    .then(response => {
                        if (response.status === 'ok') {
                            $('#checkout_modal').modal('hide');
                            this.$swal("Berhasil!", "Proses Pembuatan SO Berhasil!", "success")
                            this.$router.push({ name: 'so.detail', params: { id: response.order_id} })
                        }
                    }).catch(error => {
                        this.$swal("Terjadi Kesalahan!", "Silahkan Ulangi Kembali!", "error")
                        console.log(error);
            ...
            addCartItem() {
                ...
                if (validasi_cart === true) {
                    service.postData('/api/so/cart?user=' + this.userid, this.cartitem)
                    .then(result => {
                        if (result.status === 'ok') {
                            this.resetCart();
            ...
            getCustomer() {
                service.fetchData('/api/customer?all=true')
                .then(response => {
