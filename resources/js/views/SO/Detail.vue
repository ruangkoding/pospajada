...
<script>
    ...
    export default {
        ...
        methods: {
            getSO() {
                service.fetchData('/api/so/show/' + this.$route.params.id)
                .then(response => {
                    this.so = response;
            ...
            generateInvoice() {
                ...
                if (validasi === true) {
                    service.postData('api/so/generateinvoice?id=' + this.po.id, this.checkout)
                    .then(response => {
            ...
            rejectOrder() {   
                service.postData('api/so/rejectso?id=' + this.so.id)
                .then(response => {
                    if (response.status === 'ok') {
                        $('#reject_modal').modal('hide');
                        this.$swal("Berhasil!", "Order Berhasil Dibatalkan!", "success")
            ...
            rejectItem() {
                service.postData('/api/so/rejectitem?item=' + this.itemId)
                .then(response => {
                    if (response.status === 'ok') {
