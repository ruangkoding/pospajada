                                ...
                                <tbody>
                                    <tr v-for="v in podetail" :key="v.id" :class="{'table-warning': isReject(v.status) }">
                                        ...
                                        <td style="text-align:center;">
                                            ...
                                            <span v-if="v.status === 0 && po.status === 1"><i class="fa fa-check"></i></span>
...
<script>
    ...
    export default {
        ...
        methods: {
            getPO() {
                service.fetchData('/api/po/show/' + this.$route.params.id)
                .then(response => {
                    this.po = response;
            ...
            getPaymentMethod() {
                service.fetchData('/api/paymentmethod')
                .then(response => {
            ...
            generateInvoice() {
                ...
                if (validasi === true) {
                    service.postData('/api/po/generateinvoice?id=' + this.po.id, this.checkout)
                    .then(response => {
                        if (response.status === 'ok') {
                            $('#approval_modal').modal('hide');
                            this.$swal("Berhasil!", "Proses Pembuatan Invoice Berhasil!", "success")
                            this.$router.push({ name: 'poinvoice.detail', params: { id: response.invoice_id} })
                        }
                    }).catch(error => {
            ...
            rejectOrder() {   
                service.postData('/api/po/rejectpo?id=' + this.po.id)
                .then(response => {
                    if (response.status === 'ok') {
            ...
            rejectItem() {
                service.postData('/api/po/rejectitem?item=' + this.itemId)
                .then(response => {
                    if (response.status === 'ok') {
